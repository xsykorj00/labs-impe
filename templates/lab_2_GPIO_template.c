 ...
 * Raise FACE_TIMEOUT for a longer hold before auto-reset.
 */
...
{
    /* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! *
     * TASK 2: Handle Button Interrupts                                           *
     * 1. Check which pin caused the interrupt by reading the ISF (Interrupt      *
     *    Status Flag) in the PCR register.                                       *
     * 2. Clear the interrupt flag by writing '1' to the ISF bit.                 *
     * 3. Update the display face using set_face().                               *
     *    Note: set_face() automatically arms face_ticks = FACE_TIMEOUT so the    *
     *    display will revert to FACE_DEFAULT after the countdown expires.        *
     *                                                                            *
     * Example for UP button (PTC1):                                              *
     *   if (PORTC->PCR[1] & PORT_PCR_ISF_MASK) {                                *
     *       PORTC->PCR[1] |= PORT_PCR_ISF_MASK;  // Clear flag                  *
     *       set_face(FACE_SURPRISED);                                            *
     *   }                                                                        *
     *                                                                            *
     * Example of mapping the face reactions:                                                            *
     *   - UP     (PTC1)  ->  FACE_SURPRISED                                      *
     *   - LEFT   (PTC3)  ->  FACE_WINK_LEFT                                      *
     *   - CENTER (PTC4)  ->  FACE_HAPPY                                          *
     *   - RIGHT  (PTC5)  ->  FACE_WINK_RIGHT                                     *
     *   - DOWN   (PTC6)  ->  FACE_ASLEEP                                         *
     * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */

    // TODO: Write interrupt handlers for all 5 buttons
...
{
    /* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! *
     * TASK 1: Configure Button Pins (PTC1, PTC3, PTC4, PTC5, PTC6)               *
     * Configure the Pin Control Register (PCR) for each button to:               *
     * 1. Set MUX to 1 (GPIO).                                                    *
     * 2. Enable Pull-up resistor (PE = 1, PS = 1).                               *
     * 3. Configure Interrupt on falling edge (IRQC = 1010, which is 0xA in HEX). *
     *                                                                            *
     * Hint: You can create a common configuration variable to assign to each pin.*
     *   uint32_t btn_pcr = PORT_PCR_MUX(1) | PORT_PCR_PE_MASK | PORT_PCR_PS_MASK *
     *                    | PORT_PCR_IRQC(0xA);                                   *
     *   PORTC->PCR[1] = btn_pcr;                                                 *
     * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */

    // TODO: Configure pins PTC1, PTC3, PTC4, PTC5, PTC6
...
int main(void)
{
    ...
    /* 2. Initial state + welcome animation */
    set_face(FACE_DEFAULT);
    welcome_animation();

    /* 3. Super-loop */
    for (;;) {
        /* Step A: Safely snapshot volatile ISR variables */
        __disable_irq();
